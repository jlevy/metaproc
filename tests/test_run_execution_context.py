...
def test_mapped_composite_abort_ends_parent_attempt(tmp_path: Path) -> None:
    ...
    async def abort(**_kwargs: object) -> bool:
        raise LifecycleAbort("backend lifecycle aborted")

    async def exercise() -> None:
        context = RunExecutionContext.create(max_concurrency=1)
        try:
            with patch("metaproc.commands.run_process._execute_composite_step", abort):
                with pytest.raises(LifecycleAbort, match="backend lifecycle aborted"):
    ...
    assert status is not None
    assert status.state == "failed"
    assert status.error == "child process aborted with LifecycleAbort: backend lifecycle aborted"
    history = read_attempt_history_at(state_dir)
    assert [attempt.disposition for attempt in history] == [AttemptDisposition.lost]
...
def test_cancelling_code_command_kills_tree_before_releasing_leaf(
    ...
    try:
        ...
        assert context.leaf_semaphore is not None
        assert context.leaf_semaphore._value == 1  # noqa: SLF001 - ownership invariant
        state_dir = tmp_path / "run" / ".state" / "tasks" / "blocking-command"
        status = read_status_at(state_dir)
        assert status is not None
        assert status.state == "failed"
        history = read_attempt_history_at(state_dir)
