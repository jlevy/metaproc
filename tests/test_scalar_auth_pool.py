...
class _BlockingScalarAuthAdapter(_ScalarAuthAdapter):
    """Test adapter whose process stays alive until cancellation."""

    adapter_type = "blocking-scalar-auth-test"
    short_name = "blocking-scalar-auth-test"
    ...
    def build_command(
        ...
        script = (
            ...
            "assert os.environ['TEST_AUTH_LABEL'] == 'alt2'; "
            f"Path({variables['PID_PATH']!r}).write_text(str(os.getpid())); "
            "time.sleep(60)"
        )
        return [sys.executable, "-c", script]
...
def test_cancellation_during_scalar_credential_completion_retains_ownership(
    ...
    try:
        ...
        assert status is not None
        assert status.state == "failed"
        assert status.error is not None and "CancelledError" in status.error
        history = read_attempt_history_at(state_dir)
        assert len(history) == 1
        assert history[0].disposition is AttemptDisposition.cancelled
    finally:
        allow_completion.set()
...
def test_scalar_credential_teardown_failure_terminalizes_attempt(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    adapter = _ScalarAuthAdapter()
    monkeypatch.setitem(ADAPTER_REGISTRY, adapter.adapter_type, adapter)
    ...
    coordinator = SlotCoordinator(backend, adapter_registry={adapter.adapter_type: adapter})
    original_teardown = coordinator.teardown

    def teardown_then_raise(
        lease: SlotLease,
        *,
        failure: AuthFailureClassification | None = None,
    ) -> str | None:
        original_teardown(lease, failure=failure)
        raise RuntimeError("simulated credential teardown failure")

    monkeypatch.setattr(coordinator, "teardown", teardown_then_raise)
    runs_dir = tmp_path / "runs"
    ...
    try:
        with pytest.raises(RuntimeError, match="simulated credential teardown failure"):
            asyncio.run(
                _execute_agent_step(
                    spec=spec,
                    step_def=step_def,
                    ...
                    variables={
                        "RUNS_DIR": str(runs_dir),
                        "RUN_ID": run_id,
                        "OBSERVED_LABEL": str(tmp_path / "observed-label.txt"),
                    },
                    process_dir=tmp_path,
                    run_dir=run_dir,
                    run_id=run_id,
        ...
        assert status is not None
        assert status.state == "failed"
        assert status.error is not None and "credential teardown failure" in status.error
        history = read_attempt_history_at(state_dir)
        assert len(history) == 1
...
def test_cancelling_pooled_scalar_drains_process_before_credential_teardown(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    adapter = _BlockingScalarAuthAdapter()
    monkeypatch.setitem(ADAPTER_REGISTRY, adapter.adapter_type, adapter)
    monkeypatch.delenv("TEST_AMBIENT_AUTH", raising=False)
    backend = LocalFilesystemBackend(path=tmp_path / "pool" / "credentials.json")
    backend.upsert_entry(
    ...
    coordinator = SlotCoordinator(backend, adapter_registry={adapter.adapter_type: adapter})
    runs_dir = tmp_path / "runs"
    run_id = "cancel-pooled-scalar"
    run_dir = runs_dir / run_id
    pid_path = tmp_path / "pooled-scalar.pid"
    step_def = ProcessStep(id="scalar-agent", mode="agent", prompt_prefix="test")
    spec = ProcessSpec(name="scalar-auth", steps=[step_def])
    target = ResolvedStep(
        step_id=step_def.id,
    ...
    original_teardown = coordinator.teardown
    teardown_observations: list[tuple[int, bool]] = []
    ...
    def tracked_teardown(
        ...
        *,
        ...
        pool_owner = context.run_pool_owner
        assert pool_owner is not None and pool_owner.pool is not None
        pid = int(pid_path.read_text())
        try:
            process = psutil.Process(pid)
            process_is_live = process.is_running() and process.status() != psutil.STATUS_ZOMBIE
        except psutil.NoSuchProcess:
            process_is_live = False
        teardown_observations.append((pool_owner.pool.active_count, process_is_live))
        return original_teardown(lease, failure=failure)

    monkeypatch.setattr(coordinator, "teardown", tracked_teardown)
    ...
    async def exercise() -> None:
        try:
            ...
            assert teardown_observations == [(0, False)]
            assert context.leaf_semaphore is not None
            assert context.leaf_semaphore._value == 1  # noqa: SLF001 - ownership invariant
        finally:
