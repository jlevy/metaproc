...
else lives in [`docs/`](docs/). Each table is ordered roughly in reading order.
...
| [artifact-catalog](docs/artifact-catalog.md) | Every runtime artifact metaproc writes or reads — filename, format, schema, lifecycle, writer/readers |
...
Architecture docs live in [`docs/arch/`](docs/arch/); the canonical index with status
and ownership is
[development.md § Architecture Docs](docs/development.md#architecture-docs).
[arch-metaproc-core](docs/arch/arch-metaproc-core.md) is the master implementation
