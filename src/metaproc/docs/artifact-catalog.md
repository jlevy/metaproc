...
| --- | --- | --- | --- | --- | --- |
| `run-config.yaml` | `<run>/.state/` | ad-hoc outer dict with typed `ResourceRunSnapshot` resources block | atomic, once at creation; immutable process, run-directory, and resolved-variable identity validated on resume, excluding inputs the process declares `provenance: true`, whose launch values it keeps; recorded `step_variants` re-applied on a resume without `--step-variant` and refused when a resume passes a different set, and a config recording none adopts and records what a resume passes | `commands/run_process.py:_write_run_config` | engine resume validation, terminal resource finalizer, metabrowser, `metaproc status` |
...
| `collected-inputs.yaml` (per-step) | `<run>/.state/tasks/<step>/` | `CollectedInputsRecord` (`metaproc:CollectedInputs/0.1`) | atomic, each time the step is handed its `collect:` documents, rewritten only when its contents change | `io/state_io.py:write_collected_inputs_at` | engine resume reuse check |
...
| `dispatch-config-changes.jsonl` | `<run>/.logs/` | ad-hoc dict (typed envelope pending): `dispatch_config_change` and `provenance_advance` events | `commands/run_process.py:_record_resume_config_change`, `_record_provenance_advances` (both through `_append_dispatch_config_change_event`) | `metaproc auth usage` resume timeline |
