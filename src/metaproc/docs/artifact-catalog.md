...
| `process-status.yaml` | `<run>/.state/` | ad-hoc dict (typed envelope pending) | atomic, rewritten each DAG tick; a step stopped by the spend cap or its dispatch breaker keeps `state: failed` and carries a `stopped` block (`DispatchStop`), which the process also lists under top-level `stopped` | `commands/run_process.py:_write_process_status` | human, `metaproc status`, metabrowser |
...
| `spend-ledger.yaml` | `<run>/.state/` | `SpendLedgerRecord` (`metaproc:SpendLedger/0.1`) | atomic; written when a run with a spend cap opens its ledger, then after each finished task adds spend; present only for a run launched or resumed with a cap | `engine/spend_cap.py:SpendLedger.persist` | the ledger itself at the next launch, operator inspection |
