...
| --- | --- | --- | --- | --- | --- |
| `run-config.yaml` | `<run>/.state/` | ad-hoc outer dict with typed `ResourceRunSnapshot` resources block | atomic at creation; rewritten by a resume whose process, run directory, variables, or step variants differ, after the change is recorded; recorded `step_variants` re-applied on a resume without `--step-variant`; every other field keeps its creation value | `commands/run_process.py:_write_run_config` | engine resume validation, terminal resource finalizer, metabrowser, `metaproc status` |
...
| `dispatch-config-changes.jsonl` | `<run>/.logs/` | ad-hoc dict (typed envelope pending): `dispatch_config_change` and `launch_config_change` events | `commands/run_process.py:_record_resume_config_change`, `_record_launch_config_changes` | resource aggregator timeline, `metaproc auth usage` resume timeline |
