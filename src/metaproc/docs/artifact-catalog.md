...
| `dispatch-config-changes.jsonl` | `<run>/.logs/` | ad-hoc dict (typed envelope pending): `dispatch_config_change` and `provenance_advance` events | `commands/run_process.py:_record_resume_config_change`, `_record_provenance_advances` | resource aggregator timeline, `metaproc auth usage` resume timeline |
