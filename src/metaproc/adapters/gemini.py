...
def _build_gemini_flags(
    ...
    if "tools" in merged_config:
        ...
        if allowed:
            allow_rule = (
                "\n[[rule]]\n"
                f"toolName = {json.dumps(allowed)}\n"
                'decision = "allow"\n'
                "priority = 101\n"
            )
        policy = _materialize_temp_file(
            prefix="tools-",
            suffix=".toml",
            content=(
                "# Generated by Metaproc from the step's explicit tools boundary.\n"
                "[[rule]]\n"
                'toolName = "*"\n'
                'decision = "deny"\n'
                "priority = 100\n"
        ...
        flags.extend(["--policy", str(policy)])
...
class GeminiCliAdapter:
    ...
    def build_command(
        ...
        if prompt_file != Path("<prompt-file>") and not prompt_file.is_file():
            msg = f"gemini-cli: prompt file does not exist: {prompt_file}"
            raise FileNotFoundError(msg)
        gemini_command = ["gemini", "-p", "", *flags]
        return [
            "/bin/sh",
            "-c",
