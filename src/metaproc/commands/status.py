...
def _looks_like_run_id(target: str) -> bool:
    ...
    if len(target) > 63:
        return False
    # Scoped to the `run` prefix on purpose. Accepting any registered prefix would make
    # this depend on which domain packages happen to be imported, so an unrelated
    # `eval-v2` directory would start resolving as a remote run-id.
