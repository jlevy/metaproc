...
- **concepts** (~8.1k words): Start here. Vocabulary, ownership boundaries, step modes, optimization loops.
- **design** (~22.0k words): How Metaproc is built: spec format, runtime artifacts, adapters, robustness.
- **framework** (~7.3k words): Background theory: the general model beneath any process framework, and Metaproc's mapping.
- **operator** (~11.3k words): Runtime CLI reference: running, monitoring, and resuming processes.
...
- **arch-testing** (~1.3k words): Architecture: the test tiers, when to use each, and per-adapter credentials.
