...
name: metaproc
description: >-
  How to run, monitor, resume, and supervise any metaproc process. Use whenever launching/resuming `metaproc run-process`, running a step in isolation, or supervising a run. ALWAYS read the operator manual (`metaproc help operator`) first; this skill routes you to metaproc's built-in manuals (`metaproc help operator/developer/concepts`) and the monitoring commands instead of improvising ad-hoc bash.
