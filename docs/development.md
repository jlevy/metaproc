...
Source lives under [`src/metaproc/`](../src/metaproc/); the canonical architecture
reference is [arch-metaproc-core.md](arch/arch-metaproc-core.md).

metaproc is intentionally a thin layer above expensive things — agent SDKs, big JSONL
log streams, GCP Batch, large repository trees.
...
| [`src/metaproc/logutil/`](../src/metaproc/logutil/) | JSONL log parsing + usage extraction; the shared parser layer. |
...
| [`src/metaproc/viz/`](../src/metaproc/viz/) | Process-spec visualization (DAG + render targets). |
| [`src/metaproc/cloud/gcp/`](../src/metaproc/cloud/gcp/) | GCP Batch dispatch + worker bootstrap. |
| [`src/metaproc/io/`](../src/metaproc/io/) | Frontmatter, schema tokens, lock files, dispatch manifests. |
| [`src/metaproc/models/`](../src/metaproc/models/) | Pydantic schemas — every cross-module data contract. |
...
3. `src/metaproc/data/pi-models.default.json` — pricing rows for any models the adapter
   exposes.
4. `tests/test_adapters_<name>.py` — happy path + failure modes.
5. `tests/test_log_parsing.py` — adapter detection + event shape.
...
(`Open Questions` + `Potential Improvements`). To revise one:

```bash
tbd shortcut revise-architecture-doc
```

This is the canonical index — keep entries here in sync with the files on disk.

| Doc | Owns | Status |
| --- | --- | --- |
| [arch-metaproc-core.md](arch/arch-metaproc-core.md) | The framework itself: spec format, runtime artifacts, CLI commands, adapter wire formats, plugin protocol. Cross-refs to runpool + cloud-execution which have their own docs. | Approved |
...
- [conventions.md](conventions.md) — naming, structure, file-format rules.
- [artifact-catalog.md](artifact-catalog.md) — every file metaproc writes/reads.
- [performance-notes.md](performance-notes.md) — perf principles + worked examples.

## Cross-References

- [README.md](../README.md) — repo entry point with the full doc index.
- [MetaBrowser architecture](https://github.com/jlevy/metabrowser/blob/main/docs/architecture.md)
  — the standalone browser design and package boundary.
